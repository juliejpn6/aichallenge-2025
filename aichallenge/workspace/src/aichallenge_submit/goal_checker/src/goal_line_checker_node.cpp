...
/**
 * ゴールライン通過判定＆パラメータ変更ノード
 * 
 * 【機能】
 * - ゴールライン通過を検出してラップカウント
 * - 周回ごとにsimple_trajectory_generatorのcsv_pathを変更
 * - 周回ごとにsimple_pure_pursuitのexternal_target_velを変更
 * - 初期速度・増加幅・最高速度をlaunchファイルで設定可能
 * - 詳細なデバッグログ出力機能
 * 
 * 【変更履歴】
 * - lookahead_gain の動的変更機能を削除（時間ベースLookahead導入のため不要）
 * - external_target_vel の変更機能のみ維持
 * 
 * 【パラメータ】
...
public:
  GoalLineChecker() : Node("goal_line_checker_node")
  {
    ...
    this->declare_parameter("initial_speed", 14);      // 初期速度 (km/h)
    ...
    // Pure Pursuit用（external_target_vel更新）
...
private:
  ...
  // ===========================================
  // ★★★ 削除：lookahead_gain 計算関数 ★★★
  // 変更履歴: 時間ベースLookahead導入により不要となったため削除
  // 以前の実装：
  // double calculateLookaheadGain(double max_speed_ms)
  // {
  //   return 1.0 + (max_speed_ms - 3.0) * 0.12;
  // }
  // ===========================================
  ...
  void updateAllParametersAsync()
  {
    ...
    // 目標速度の計算
    // -----------------------------------------------
    int target_speed_kmh = initial_speed_ + lap_count_ * speed_increment_;

    if (enable_debug_) {
      RCLCPP_DEBUG(this->get_logger(), 
                   "[DEBUG] 速度計算: %d + %d * %d = %d km/h", 
    ...
    // ========================================
    // 2. Pure Pursuit のexternal_target_velを更新
    // ★★★ 変更：lookahead_gain更新を削除 ★★★
    // ========================================
    if (pure_pursuit_param_client_->service_is_ready()) {
      // 変更履歴: lookahead_gainパラメータ更新を削除（時間ベース方式に変更のため）
      auto pure_pursuit_params = std::vector<rclcpp::Parameter>{
        rclcpp::Parameter("external_target_vel", target_speed_ms)
        // 削除: rclcpp::Parameter("lookahead_gain", new_lookahead_gain)
      };
      
      pure_pursuit_param_client_->set_parameters(
        pure_pursuit_params,
        [this, target_speed_kmh, target_speed_ms](
          std::shared_future<std::vector<rcl_interfaces::msg::SetParametersResult>> future
        ) {
          try {
            auto results = future.get();
